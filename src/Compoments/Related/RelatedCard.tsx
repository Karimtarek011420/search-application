...
interface Job {
  ...
  SetSkills: React.Dispatch<
    React.SetStateAction<{ SkillsData: any[]; SkillsInfo: any[] }>
  >;
  ATTR: string[];
}
...
const RelatedCard = React.memo((props: Job) => {
  ...
  const [SkillsInfo, setSkillsInfo] = useState<
    { importance: number; level: string; type: string }[]
  >([]);

  useEffect(() => {
    const newJobIDs = Jobs.flatMap((jobArray: { id: string }[]) =>
      jobArray.map((job: { id: string }) => job.id)
    );
    const newSkillIDs = Skills.flatMap((skillArray: { id: string }[]) =>
      skillArray.map((skill: { id: string }) => skill.id)
    );
    setJobIDs(newJobIDs);
    ...
    const fetchJobs = async () => {
      if (jobIDs.length) {
        try {
          ...
          const jobDetails = JobsResponses.map(
            (response) => response.data.data.job.attributes
          );
    ...
    const fetchSkills = async () => {
      if (skillIDs.length) {
        try {
          ...
          const skillsDetails = skillResponses.map(
            (response) => response.data.data.skill.attributes
          );
